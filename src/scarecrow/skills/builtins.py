...
- 大文件用 `nrows=1000` 先采样
- 不要随意覆盖用户已有变量；如果需要新变量，优先使用语义化名称，例如 `primary_df`、`events_df`、`metrics_df`
...
“这份数据有 10000 行、12 列，其中 7 个数值列、5 个类别列。部分字段存在少量缺失，某个标识列看起来可以作为主键或连接键。下一步可以继续看目标变量分布、重复记录或跨表一致性。”
